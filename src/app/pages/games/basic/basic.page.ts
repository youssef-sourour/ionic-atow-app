...
export class BasicPage implements OnInit {
    textarea = "../../../assets/imgs/gameplay/transparent_button.png";
    isActiveText = false;
    messageMinMaxLetter = '';
    messageWordRequired = '';
    messageLetterRequired = '';
    messageMinMaxWords = '';
    
    public totalGameLevelScores: any = 0;
    public limitScoreToUnlockNewGame: any;
    ...
    public notHint = "";
    public wrapperBoxWidth: any;
    wrapperItemHeight: any;
    public wiBoxWidth: any;
    public wiBoxHeight: any;
    deviceHeight: any;
    ...
    ionViewWillEnter() {
        ...
        this.authService.getUser().subscribe(
            data => {
                ...
                let modalContent = `<div id="myModal" class="at-modal">
                <!-- Modal content -->
                <div class="at-modal-content">
                  
                  <h2 style="margin-top: 21px;">YOUR COINS</h2>
                  
                  <p class="your-coin">${this.twirlAndTipValue}</p>
                ...
                if (params && params.data) {
                    ...
                    if (this.continuousGame === true) {
                        this.storage.get("save_game").then(data => {
                            ...
                            if (this.saveGameData.finalLetterOfRow)
                                this.finalLetterOfRow = this.saveGameData.finalLetterOfRow.toUpperCase();
                            if (this.saveGameData.wordGridResult)
                                this.wordGridResult = this.saveGameData.wordGridResult;
                            if (this.saveGameData.wordLibArr)
                                this.wordLibArr = this.saveGameData.wordLibArr;
                            if (this.saveGameData.scoreLibArr)
                                this.scoreLibArr = this.saveGameData.scoreLibArr;
                            if (this.saveGameData.focusI)
                                this.focusI = this.saveGameData.focusI;
                            if (this.saveGameData.focusJ)
                                this.focusJ = this.saveGameData.focusJ;
                            if (this.saveGameData.wordResult)
                                this.wordResult = this.saveGameData.wordResult;
                            if (this.saveGameData.wordRowResult)
                                this.wordRowResult = this.saveGameData.wordRowResult;
                            if (this.saveGameData.hintString)
                                this.hintString = this.saveGameData.hintString;
                            this.toogleOpenKeyboard();
                            this.playWithTheme(this.saveGameData.gameTheme);
                            this.getHeightWidth();
                            let checkWiBoxDidMount = setInterval(() => {

                                let innerbox = $('.wi-box').filter((i, e) => {
    ...
    initializeApp() {
        this.platform
            ...
            .then(() => {
                console.log(
                    "render complete" +
                    this.element.nativeElement.querySelector('[id="c' + 0 + 0 + '"]')
                        .innerHTML
                );
                ...
                .subscribe(data => {
                    ...
                    if (this.focusI === this.numGameWord - 1) {
                        ...
                        for (let x = 0; x < this.wordLibArr.length; x++) {
                            ...
                            if (
                                wordStr[this.numGameLetter - 1] ===
                                this.firstLastLetter.toLowerCase()
                            ) {
                        ...
                        this.database.updateScores(this.gameLevel, this.score, 1).subscribe(
                            data => {
                                ...
                                if (this.gameLevel == this.user.game_level_unlocked) {
                                    this.user.game_level_unlocked =
                                        this.user.game_level_unlocked + 1;
                                }
    ...
    getScoreLibArr() {
        return new Promise((resolve, reject) => {
            this.database.getScoreLibArr().subscribe(data => {
                // get Lib data
    ...
    defineGridResultArr() {
        ...
        this.wordGridResult[this.numGameWord - 1][
            this.numGameWord - 1
        ] = this.firstLastLetter;
    }
    ...
    getHeightWidth = () => {
        let glutterHorizontal = 5 * (this.numGameLetter - 1);
        let glutterVertical = 5 * (this.numGameWord - 1);

        this.wrapperBoxWidth = $(window).width() - 100;
        this.wrapperItemHeight = this.deviceHeight - 330;

        this.wiBoxWidth =
            (this.wrapperBoxWidth - glutterHorizontal) / this.numGameLetter;
        this.wiBoxHeight =
            (this.wrapperItemHeight - glutterVertical) / this.numGameWord;
    ...
    async checkResultOfRow() {
        ...
        if (
            this.focusI == this.numGameWord - 1 &&
        ...
        if (this.checkWordInLib(this.wordResult)) {
            ...
            let label = $("#c" + this.focusI + this.focusJ);
            ...
            if (this.isHintUse === true) {
                ...
                // this.hintString += this.letterSelected.toLowerCase();
                this.notHint += this.letterSelected.toLowerCase();
            }
            ...
            if (
                ...
                this.endGame = true;
                if (
                    this.score + this.totalGameLevelScores >=
            ...
            if (this.focusJ === this.numGameLetter - 1) {
                // this.loading.present();
                this.finalLetterOfRow = this.element.nativeElement.querySelector(
                    '[id="c' + this.focusI + (this.numGameLetter - 1) + '"]'
                ).innerHTML;
                //put to grid resultArr
                this.wordGridResult[this.focusI][
                    this.numGameLetter - 1
                ] = this.letterSelected;
                this.focusJ = 0;
                this.focusI++;
                this.hintString = "";
        ...
        //if input word not include in arr (reset row)
            ...
            this.wordResult = "";
            console.log(
                "wrong word! please try again! focus ij:" + this.focusI + this.focusJ
            );
    ...
    async calcScore(wordNeedCalc) {
        for (let x = 0, lenx = wordNeedCalc.length; x < lenx; x++) {
            for (let y = 0, leny = this.scoreLibArr.length; y < leny; y++) {
                if (
                    wordNeedCalc[x].replace(/[^\w\s]/gi, "").toLowerCase() ===
    ...
    async useHint() {
        if (this.twirlAndTipValue >= 25) {
            ...
            //     }
            // }
            this.focusJ =  this.hintString.length + this.notHint.length +1;
            this.setFocus(this.focusI, this.focusJ);
            this.isHintUse = true;
            if (this.focusI === this.numGameWord - 1) {
                for (let x = 0; x < this.wordLibLastRowArr.length; x++) {
                    ...
                    let wordHint = "";
                    if (this.focusI === 0) {
                        wordHint = this.firstLastLetter.toLowerCase() + this.hintString.concat(this.notHint);
                    } else {
                        wordHint = this.finalLetterOfRow.toLowerCase() + this.hintString.concat(this.notHint);
                    }
                    console.log("word:" + word);
                    console.log("wordHint:" + wordHint);
                    console.log("word.indexOf(wordHint):" + word.indexOf(wordHint));
                    if (
                        word.indexOf(wordHint) === 0 &&
    ...
    displayKeyboard() {
        if (
            this.element.nativeElement.querySelector('[id="c01"]').innerHTML === ""
        ) {
            this.toogleOpenKeyboard();
