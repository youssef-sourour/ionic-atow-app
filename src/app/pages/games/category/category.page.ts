...
export class CategoryPage implements OnInit {
    ...
    getWordLibDBArr(numGameLetter, firstLastLetter, category, theme) {
        return new Promise((resolve, reject) => {
            this.database.getWordLibByCategory(numGameLetter, firstLastLetter, category).subscribe(
                data => {
                    // get Lib data
    ...
    async selectGameData(form: NgForm) {
        ...
        this.getWordLibDBArr(this.numGameLetter, this.firstLastLetter, this.category, 'spring').then(
            data => {
                console.log(data);
            },
            error => {
    ...
    async checkResultOfRow() {
        ...
        if (this.checkWordInLib(this.wordResult)) {
            ...
            if (this.focusJ === this.numGameLetter - 1) {
                ...
                this.getWordLibDBArr(this.numGameLetter, this.finalLetterOfRow,this.category, -1).then(
                    data => {
                        console.log(data);
                    },
                    error => {
