...
export class AssortedPage implements OnInit {
    public wordsLettersForm: FormGroup;
    public numRowLettersCount: number = 1;

    public isGameConfigHidden = true;
    public totalGameLevelScores: any = 0;
    public limitScoreToUnlockNewGame: any;
    public limitScoresArr: any;
    public gameLevel: any;
    public gameType: any;
    public disableBtn: any = 0;
                ...
                if (params && params.data) {
                    ...
                    if (this.continuousGame === true) {
                        this.storage.get('save_game').then((data) => {
                            ...
                            this.arrayK =this.saveGameData.arrayK;
    ...
    async playWithTheme(theme) {
        ...
        for(let x=0; x < this.numGameWord; x++){
                ...
                data => {
                    ...
                    if (this.focusI === this.numGameWord - 1) {
                        ...
                        for (let x = 0; x < this.wordLibArr.length; x++) {
                            ...
                            if (wordStr[this.arrayK[this.focusI] - 1] === this.firstLastLetter.toLowerCase()) {
    ...
    async selectGameData(form) {
        ...
        this.getWordLibDBArr(this.arrayK[0], this.firstLastLetter, 'spring').then(
            data => {
                console.log(data);
            },
            error => {
    ...
    async checkResultOfRow() {
        ...
        if (this.checkWordInLib(this.wordResult)) {
            ...
            if (this.focusJ === this.arrayK[this.focusI] - 1) {
                ...
                this.getWordLibDBArr(this.arrayK[this.focusI], this.finalLetterOfRow, -1).then(
                    data => {
                        console.log(data);
                    },
                    error => {
        ...
        //if input word not include in arr (reset row)
            ...
            if (this.focusI === this.numGameWord - 1) {
                for (let j = 1; j <= this.arrayK[this.focusI] - 2; j++) {
                    this.focusJ = j;
                    this.setFocus(this.focusI, this.focusJ);
                ...
                for (let j = 1; j <= this.arrayK[this.focusI] - 1; j++) {
                    this.focusJ = j;
                    this.setFocus(this.focusI, this.focusJ);
                    if (this.hintString[j - 1]) {
                        console.log('j:' + j);
